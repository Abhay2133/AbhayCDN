...
function parse_val(sn, val, debug)//sn mean styleName var in Loop of apply_CSS function
{
	...
	if(fc == "bdr")
	{
		if(sc == "C" || tc == "C")		// for color we use C for prop2
			parsed_val = v; // ex : bdr-C-green
		else		
		if(!Object.keys(prop2).includes(sc) && tc == v)
			parsed_val = `${v} solid`
		else
	...
	if( v.startsWith("_"))
	{
		...
		parsed_val = v.replace("_", "-");
	}
    else
    if(z == "%" || v.includes("vh") || v.includes("rem") || "ff,trans,left,right,btm,top".split(",").includes(fc))
    {
        parsed_val = v
    }
    else
